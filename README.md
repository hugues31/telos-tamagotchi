...
Momo's soul lives in [`telos/`](telos/): 9 notions, 12 intents in EARS
form, 17 scenarios each proven by a sealed red-then-green test witness,
and 2 constraints with executable checks. The Python in
...
- **Run the deterministic driver** — it parses the machine-readable
  ...
  `--compare` then requires every replayed file to match this
  repository byte for byte — `telos/telos.lock` is a table of git blob
  OIDs, so lock equality alone pins every sealed byte of spec and code.

## This demo is also an end-to-end test

CI replays the entire story from an empty directory on every push
...
([telos.yml](.github/workflows/telos.yml), generated by `telos init
--ci github` and never edited) gates every push on `telos check
--sealed`. The `pages` job publishes `telos view --export` — browse
Momo's soul at
...
```
telos/            the sealed spec — notions, intents, constraints, bindings, lock
